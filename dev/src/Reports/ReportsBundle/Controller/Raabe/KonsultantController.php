...
class KonsultantController extends Controller {
    ...
    public function getTableBody() {
        ...
        foreach ( $combine['work_time'] as $row ) {
            ...
            $count_target += $target;
            $count_sale += $sale;
            $count_time += round($row['agent_work_time'], 2);
            $count_closed += $closed;
        }
        
        $table_body .= '<tr style="font-weight: bold"><td>#</td><td>SUMA</td><td>' . $count_target . '</td>'
                . '<td>' . $count_sale . '</td><td>-</td><td>' . $count_time . '</td><td>' . $count_closed . '</td><td>-</td></tr>';
