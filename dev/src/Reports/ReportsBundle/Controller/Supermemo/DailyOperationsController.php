...
class DailyOperationsController extends Controller {
    ...
    public function getTableHead() {
        $table_head = '<tr><th>Ilość połączeń</th><th>Ilość minut komórkowe</th><th>Ilość minut stacjonarne</th>'
                ...
                . '<th>Ilość umówionych spotkań</th><th>Konwersja efektu do rekordów zamkniętych</th><th>Średni czas rozmowy</th>'
    ...
    public function getTableBody() {
        ...
        $table_body = '<td>' . $count_calls . '</td><td>' . round(($count_gsm_minutes/60), 2) . '</td><td>' . round(($count_minutes/60), 2) . '</td>'
                ...
                round((($count_minutes/60) * 0.04)+(($count_gsm_minutes/60)*0.18), 2) . ' zł</td><td>' . round((($count_gsm_minutes/60)+($count_minutes/60))*0.69, 2). ' zł</td>'
                . '<td>' . $count_closed . '</td><td>' . $count_orders . '</td><td>' . round(($count_orders/$count_closed) * 100, 2) . '%</td>'
