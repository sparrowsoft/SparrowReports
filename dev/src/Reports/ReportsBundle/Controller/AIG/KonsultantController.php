...
class KonsultantController extends Controller {
    ...
    public function Orders($id) {
        $closed_query = "SELECT COUNT(calllog_agentid) AS ORDERS FROM cc_calllog WHERE calllog_agentid = '" . $id . "' 
                AND calllog_callcode_id = 505 AND calllog_campaign = '" . $this->campaign_id[0]['campaign_id'] . "'
    ...
    public function WorkTime($id) {
        ...
        foreach ( $closed->fetchAll() as $date ) {
            ...
            if ( $date['calllog_callcode_id'] != NULL ) {
                if ( $date['calllog_callcode_id'] === 505 OR $date['calllog_callcode_id'] === 27 
        ...
        return array('target' => $counter, 'work_time' => round(($work_time/3600), 2), 'calls' => $calls);
    }
    ...
    public function getTableHead() {
        $table_head = '<tr><th>#</th><th>Agent</th><th>Osób docelowych</th>
                <th>Zamówień</th><th>Skuteczność do osoby docelowej</th><th>Ilość połączeń</th>
                <th>Czas rozmów</th><th>Zamknięć</th><th>Efektywność</th></tr>';
    ...
    public function getTableBody() {
        ...
        foreach ( $this->Agents() as $record ) {
            ...
            $table_body .= '<tr><td>' . $count_rows . '</td><td>' . $agent[0]['user_name'] . ' ' . $agent[0]['user_surname'] . '</td>'
                    . '<td>' . $work_time['target'] . '</td><td>' . $orders[0]['orders'] . '</td><td>' . round($target_efficacy, 2) . '%</td>'
                    . '<td>' . $work_time['calls'] . '</td><td>' . $work_time['work_time'] . '</td><td>' . $closed[0]['closed'] . '</td><td>' . round($efficacy, 2) . '%</td></tr>';
        ...
        $table_body .= '<tr style="font-weight: bold"><td>#</td><td>SUMA</td><td>' . $count_target . '</td><td>' . $count_orders. '</td>'
                 . '<td>' . round($target_all_efficacy, 2) . '%</td><td>' . $count_calls . '</td><td>' . $count_work_time . '</td><td>' . $count_closed . '</td><td>' . round($all_efficacy, 2) . '%</td></tr>';
